...
function App() {
  ...
  const handleSignUp = () => {
    if (email.trim() === "") {
      toast.error("Please enter your email to sign up!"); // Show error message
    } else {
      navigate(`/signup?email=${encodeURIComponent(email)}`); // Navigate to signup with email
    }
  };
      ...
      <div className="relative z-10 flex flex-col items-center text-center max-w-[821px] px-4">
        ...
        <div className="relative w-full md:mt-[2em] max-w-[360px] sm:max-w-[700px] md:w-[600px] md:h-[57px] h-[48px] sm:h-[57px] mx-auto">
          <input
            ...
            className="w-full h-full bg-white placeholder:text-[14px] lg:placeholder:text-[20px] focus:outline-gray-200 rounded-[10px] px-4 shadow-2xl md:scale-100"
          />
          <motion.button
            ...
            onClick={handleSignUp} // Trigger navigation or toast
          >
            {t.signUp}
          </motion.button>
        </div>
