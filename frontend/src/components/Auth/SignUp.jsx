...
function SignUp() {
  ...
  useEffect(() => {
    ...
    if (emailFromQuery) {
      setValue('email', decodeURIComponent(emailFromQuery));
    }
  ...
  const onSubmit = async (data) => {
    ...
    const userData = {
      ...
      user_role: data.userType, // "vendor" or "market_owner"
      avatar: '',
    };
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="relative">
            <input
              ...
              className={`w-full px-4 py-3 bg-gray-100 rounded-lg pr-10 border-1 ${
                errors.name ? 'border-red-500' : 'border-[#DCDCDC]'
          ...
          <div className="relative flex space-x-2">
            <div className="relative w-1/4">
              <div
                className="w-full px-2 py-3 bg-gray-100 rounded-lg cursor-pointer border-1 border-[#DCDCDC] flex justify-between items-center hover:bg-gray-200 focus:outline-none focus:ring-1 focus:ring-[#EA7A39] transition-all text-sm"
                onClick={() => setIsCountryDropdownOpen(!isCountryDropdownOpen)}
              >
                <span className="text-gray-700">
                  {COUNTRY_OPTIONS.find((opt) => opt.value === selectedCountryCode)?.value ||
                    selectedCountryCode}
                </span>
              ...
              {isCountryDropdownOpen && (
                <div className="absolute z-10 w-64 mt-1 bg-white rounded-xl shadow-lg border border-gray-200 max-h-60 overflow-y-auto">
                  ...
                  {filteredCountries.map((option) => (
                    <div
                      key={option.value}
                      className="px-2 py-1 text-gray-700 hover:text-white hover:bg-[#EA7A39] cursor-pointer transition-colors duration-150 text-sm flex justify-between items-center"
            ...
            <div className="relative w-3/4">
              <input
                ...
                className={`w-full px-4 py-3 bg-gray-100 rounded-lg pr-10 border-1 ${
                  errors.phoneNumber ? 'border-red-500' : 'border-[#DCDCDC]'
                } focus:outline-none focus:ring-1 focus:ring-[#EA7A39] transition-all`}
              />
          ...
          <div className="relative">
            <div
              className={`w-full px-4 py-3 bg-gray-100 rounded-lg cursor-pointer border-1 ${
                errors.userType ? 'border-red-500' : 'border-[#DCDCDC]'
              } flex justify-between items-center hover:bg-gray-200 focus:outline-none focus:ring-1 focus:ring-[#EA7A39] transition-all`}
              onClick={() => setIsDropdownOpen(!isDropdownOpen)}
            >
              <span className="text-gray-700">
                {userTypes.find((type) => type.value === register('userType').value)?.label ||
                  'Want to sign up as'}
              </span>
            ...
            {isDropdownOpen && (
              <div className="absolute z-10 w-full mt-1 bg-white rounded-xl shadow-lg border border-gray-200">
                {userTypes.map((type) => (
                  <div
                    key={type.value}
                    className="px-4 py-2 text-gray-700 hover:text-white hover:bg-[#EA7A39] cursor-pointer transition-colors duration-150"
          ...
          <p className="text-left text-sm text-gray-600">
            ...
            <Link to="/login" className="text-[#EA7A39] hover:text-orange-500 font-medium">
              Log In
            </Link>
          </p>
        </form>
