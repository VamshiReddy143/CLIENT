...
import useAuthStore from '../store/authSlice'; // Import Zustand store to access user data

const SearchPage = () => {
  const location = useLocation();
  const { user } = useAuthStore(); // Get user data from Zustand store
  ...
  // Fallback avatar if user.avatar is not available
  const defaultAvatar =
        ...
        <div className="flex gap-4 items-center">
          ...
          <div className="flex bg-white p-3 gap-2 rounded-xl items-center">
            ...
            <div className="flex items-center gap-1">
              <img
                src={user?.avatar || defaultAvatar} // Use user's avatar or fallback
                alt="Profile"
                className="w-8 h-8 rounded-full object-cover"
                onError={(e) => {
                  e.target.src = defaultAvatar; // Fallback if the avatar fails to load
                }}
              />
              <span className="text-gray-700">{user?.name || 'User'}</span> {/* Dynamic user name */}
              <svg
                width="8"
                height="8"
                viewBox="0 0 8 8"
